        ...
        <Container bgColor={useColorModeValue("primary.300", "primary.700")} borderRadius={10}>
            ...
            <Box>
                {isLoading ? (
                    ...
                    questions.length >= 1 && <ReactJson src={questions} collapsed={1} theme={"monokai"} displayDataTypes={false} name={"trivia"} style={{ borderRadius: 10 }} />
                )}
            </Box>
        </Container>
    );
...
export const hobbies = (isFrameLoading: boolean, setFrameLoading: (l: boolean) => void): Parameters<typeof Project>[0][] => [
    {
        ...
        devStack: [Libs.NodeJS.icon, Libs.Typescript.icon, Libs.JavaScript.icon, SecondaryLibs.Postgres.icon, SecondaryLibs.Fastify.icon, SecondaryLibs.ESLint.icon, SecondaryLibs.Redis.icon].map(
            (devIcon, indx) => React.createElement(devIcon, { key: `${indx}-rocket-dev-stack` }),
        ),
        features: [
            <Feature
                key={"TriviaRepo"}
            ...
            />,
            <Feature key={"TrFast"} icon={<SecondaryLibs.Fastify.icon />} content={<Text fontWeight={600}>Fastify: used for its low-overhead server, routes, validations, and auth</Text>} />,
            <Feature key={"TrSwag"} icon={<SecondaryLibs.Swagger.icon />} content={<ExternalLink to={"https://trivia-art.herokuapp.com/documentation"} text={"Swagger Docs"} />} />,
        ],
        preview: <Trivia />,
    },
    {
        ...
        preview: (
            <AspectRatio w={400} h={isFrameLoading ? 400 : 600}>
                <>
                    {isFrameLoading && <Spinner thickness="4px" speed="1.85s" emptyColor="primary.200" color="primary.500" size="xs" />}
            ...
            >
                <VStack>
                    <a href="https://github.com/petarzarkov/hotstuff/actions/">
                        <img src="https://github.com/petarzarkov/hotstuff/actions/workflows/build.yml/badge.svg?branch=main" alt="Build status" />
                    </a>
